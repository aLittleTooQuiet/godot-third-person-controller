...
@export var joypad_h_sensitivity: float = 0.06
@export var joypad_v_sensitivity: float = 0.03
...
@export var invert_v_look: bool = true
...
func _physics_process(delta: float) -> void:
	...
	_handle_controller_camera_movement()
...
func _handle_ground_movement() -> void:
	...
	direction = direction.rotated(Vector3.UP, pivot_horizontal.rotation.y)
	if direction:
		avatar.look_at(global_position - direction, Vector3.UP, true)
